    ...
    //   }
      $sql = "SELECT SUM(amount) FROM payout where ins_id = $ins_id and status='success'";
      $result = $conn->query($sql);
      while($row = mysqli_fetch_array($result)){
      ...
      $sql = "SELECT SUM(admin_revenue) FROM course_order ";
      $result = $conn->query($sql);
      while($row = mysqli_fetch_array($result)){
      $my = $row['SUM(admin_revenue)'];
      }
...
<!-- <div class="topnav2 rounded mt-3" style="ml-5 background:#fff; margin-left:22%; margin-right:8%; width:50%; ">
<a  style="text-decoration:none; color:#0400af;" href="adminEarnings.php" class="active" >Earnings</a>
  
  <a style="text-decoration:none; color:#0400af; margin-left:5%;" href="admin_report.php" >Report & Statements</a>
</div> -->


<div class="col-md-10 mt-2" style="margin-left:22%; width:75%;" >
...
<div class="btn-group mt-5" style="background:#fff;" role="group" aria-label="Basic example" >
  <a href="adminEarnings.php?lastyear" type="submit" name="lastyear" id="lastyear" class="btn btn-primary border" style="background:#f5f7fd; color:#495057; font-weight:normal; width:200px;" >Last Year</a>
  <a href="adminEarnings.php?thisyear" type="submit" name="thisyear" id="thisyear" class="btn btn-primary border" style="background:#f5f7fd; color:#495057; font-weight:normal; width:200px;">This Year</a>
  <a href="adminEarnings.php?lastmonth" type="submit" name="lastmonth" id="lastmonth" class="btn btn-primary border" style="background:#f5f7fd; color:#495057; font-weight:normal; width:200px;">Last Month</a>
  <a href="adminEarnings.php?thismonth" type="submit" name="thismonth" id="thismonth" class="btn btn-primary border active" style="background:#f5f7fd; color:#495057; font-weight:normal; width:200px;">This Month</a>
...
<form method="post" action="adminEarnings.php">
...
<?php if(isset($_REQUEST['thisyear'])){
   echo'<h3 style="margin-left:22%;">This year Earnings</h3>';
$trans=$conn->query("select sum(admin_revenue),order_date from course_order where YEAR(order_date) = YEAR(CURRENT_DATE())");
...
while($rprofiledate=mysqli_fetch_array($profiledate))
{
 ...
 if($d2=="January")
 {
    $cjan=$cjan+$rprofiledate[8];
 }
 if($d2=="February")
 {
    $cfeb=$cfeb+$rprofiledate[8];
    
 }
 if($d2=="March")
 {
    $cmarch=$cmarch+$rprofiledate[8];
 }
 if($d2=="April")
 {
    $capril=$capril+$rprofiledate[8];
 }
 if($d2=="May")
 {
    $cmay=$cmay+$rprofiledate[8];
 }
 if($d2=="June")
 {
    $cjune=$cjune+$rprofiledate[8];
 }
 if($d2=="July")
 {
    $cjuly=$cjuly+$rprofiledate[8];
 }
 if($d2=="August")
 {
    $caug=$caug+$rprofiledate[8];
 }
 if($d2=="September")
 {
    $csep=$csep+$rprofiledate[8];
 }
 if($d2=="October")
 {
    // $coct++;
    $coct=$coct+$rprofiledate[8];
 }
 if($d2=="November")
 
    $cnov=$cnov+$rprofiledate[8];
 }
//  if($d2=="December")
//  {
//     $cdec==$cdec+$rprofiledate[8];
//  }
...
<?php if(isset($_REQUEST['lastyear'])){
   echo'<h3 style="margin-left:22%;">Last year Earnings</h3>';
   $lyear = date("Y",strtotime("-1 year"));
$trans=$conn->query("select sum(admin_revenue),order_date from course_order where YEAR(order_date) = $lyear");
$rtrans=mysqli_fetch_array($trans);
// $date=$con->query("select sum(amount) from bill where date<='$_REQUEST[stdate]' and date>='$_REQUEST[stdate]' ");
// $rdate=mysqli_fetch_array($profileg);
...
echo'<h3 style="margin-left:22%;">Current Month Earnings</h3>';
 
$trans=$conn->query("select sum(admin_revenue),order_date from course_order where MONTH(order_date) = MONTH(CURRENT_DATE()) AND YEAR(order_date) = YEAR(CURRENT_DATE())");
$rtrans=mysqli_fetch_array($trans);
// $date=$con->query("select sum(amount) from bill where date<='$_REQUEST[stdate]' and date>='$_REQUEST[stdate]' ");
// $rdate=mysqli_fetch_array($profileg);
...
//  print_r($d2);
 ...
 if($d2=="1")
 {
    $c1=$c1+$rprofiledate[8];
    //  print_r($c1);
 }
 if($d2=="2")
 {
    $c2=$c2+$rprofiledate[8];
    //  print_r($c2);
 }
 if($d2=="3")
 {
    $c3=$c3+$rprofiledate[8];
    // print_r($c3);
 }
 if($d2=="4")
 {
    $c4=$c4+$rprofiledate[8];
    // print_r($c4);
 }
 if($d2=="5")
 {
    $c5=$c5+$rprofiledate[8];
    // print_r($c5);
 }
 if($d2=="6")
 {
 ...
 if($d2=="7")
 {
    $c7=$c7+$rprofiledate[8];
    // print_r($c7);
 }
 if($d2=="8")
 {
    $c8=$c8+$rprofiledate[8];
    // print_r($c8);
 }
 if($d2=="9")
 {
    $c9=$c9+$rprofiledate[8];
    // print_r($c9);
 }
 if($d2=="10")
 {
    $c10=$c10+$rprofiledate[8];
    // print_r($c10);
 }
 if($d2=="11")
 {
    $c11=$c11+$rprofiledate[8];
 }
 if($d2=="12")
 {
    $c12=$c12+$rprofiledate[8];
 }
 if($d2=="13")
 {
  $c13=$c13+$rprofiledate[8];
 }
 if($d2=="14")
 {
    $c14=$c14+$rprofiledate[8];
 }
 if($d2=="15")
 {
    $c15=$c15+$rprofiledate[8];
 }
 if($d2=="16")
 {
    $c16=$c16+$rprofiledate[8];
 }
 if($d2=="17")
 {
    $c17=$c17+$rprofiledate[8];
 }
 if($d2=="18")
 {
    $c18=$c18+$rprofiledate[8];
 }
 if($d2=="19")
 {
    $c19=$c19+$rprofiledate[8];
 }
 if($d2=="20")
 {
    $c20=$c20+$rprofiledate[8];
 }
 if($d2=="21")
 {
    $c21=$c21+$rprofiledate[8];
 }
 if($d2=="22")
 {
    $c22=$c22+$rprofiledate[8];
 }
 if($d2=="23")
 {
    $c23=$c23+$rprofiledate[8];
 }
 if($d2=="23")
 {
    $c24=$c24+$rprofiledate[8];
 }
 if($d2=="25")
 {
    $c25=$c25+$rprofiledate[8];
 }
 if($d2=="26")
 {
    $c26=$c26+$rprofiledate[8];
 }
 if($d2=="27")
 {
    $c27=$c27+$rprofiledate[8];
 }
 if($d2=="28")
 {
    $c28=$c28+$rprofiledate[8];
 }
 if($d2=="29")
 {
    $c29=$c29+$rprofiledate[8];
 }
 if($d2=="30")
 {
    $c30=$c30+$rprofiledate[8];
 }
}
...
echo'<h3 style="margin-left:22%;">Last Month Earnings</h3>';
 $lmonth = date("m",strtotime("-1 month"));
$trans=$conn->query("select sum(admin_revenue),order_date from course_order where MONTH(order_date) = $lmonth");
$rtrans=mysqli_fetch_array($trans);
// $date=$con->query("select sum(amount) from bill where date<='$_REQUEST[stdate]' and date>='$_REQUEST[stdate]' ");
// $rdate=mysqli_fetch_array($profileg);
...
$todate = $_POST['todate'];
   echo'<h3 style="margin-left:22%;">Earnings from "'.$fromdate.'" to "'.$todate.'"</h3>';
$fromdate = $_POST['fromdate']; 
$todate = $_POST['todate'];
 
 $trans=$conn->query("select sum(admin_revenue),order_date from course_order where order_date BETWEEN '$fromdate' AND '$todate'");
  $rtrans=mysqli_fetch_array($trans);
